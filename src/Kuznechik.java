...
public class Kuznechik {
    ...
    public byte[] encrypt(byte[] input, Mode mode) {
        return switch (mode) {
            ...
            case CFB -> encryptCFB(input);
            case OFB -> encryptOFB(input);
            case CTR -> encryptCTR(input);
    ...
    public byte[] decrypt(byte[] input, Mode mode) {
        return switch (mode) {
            ...
            case CFB -> decryptCFB(input);
            case OFB -> decryptOFB(input);
            case CTR -> decryptCTR(input);
    ...
    private byte[] encryptCBC(byte[] input) {
        ...
        byte[][] outputBlocks = splitData(input);
        outputBlocks[0] = encrypt(XOR(iVector, outputBlocks[0]));
    ...
    private byte[] decryptCBC(byte[] input) {
        ...
        outputBlocks[0] = XOR(iVector, decrypt(inputBlocks[0]));
        for (int i = 1; i < size; i++)
            outputBlocks[i] = XOR(inputBlocks[i - 1], decrypt(inputBlocks[i]));
        byte[] temp = concatData(outputBlocks);
        byte[] output = new byte[stopCBC];
    ...
    private byte[] encryptCFB(byte[] input) {
        ...
        byte[][] inputBlocks = splitData(input);
        byte[][] outputBlocks = new byte[size][16];
        outputBlocks[0] = XOR(encrypt(iVector), inputBlocks[0]);
        for (int i = 1; i < size; i++)
